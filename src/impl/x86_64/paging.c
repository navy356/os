...
uint32_t getPhysical(uint32_t address, struct page_t * page_table)
{
    ...
    uint32_t address1 = getEntryAddress(page_table,offset0);
    uint32_t address2 = getEntryAddress((struct page_t *)(uintptr_t)address1,offset1);
    uint32_t address3 = getEntryAddress((struct page_t *)(uintptr_t)address2,offset2);
...
uint32_t getEntryAddress(struct page_t * page_table, uint8_t offset)
{
    ...
    uint32_t entry = (page_entry.entry & ADDR_MASK);
    return entry;
}